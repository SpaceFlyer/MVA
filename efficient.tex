...
%is $r_i$. 
By efficiency, we either have $r_k = a_k = 0$ for some $k$, or $\lim_{i
  \rightarrow \infty} r_i = 0$. WHAT IF $F(0.5) = 0$ WITH INFINITE MANY ROUNDS?
...
The following equations connects $a_i$ and $r_i$. For all $i, i+1$:
...
thresholds $a_1, a_2, \ldots$ where the bidder with valuation at least $a_i$
but less than $a_{i-1}$ (presumably $a_0 = 1$) will bid in round $i$ 
\end{theorem}

\begin{proof}
By equation \ref{eq:MVA_eq_relation}, we have $r_i P(a_i) = \int_{0}^{a_i}
x\,p(x)dx$ for all $i$ (even if
$a_i = a_k = 0$ by efficiency constraint). Thus the right
