...
one bidding is required to terminate so $C \geq \rho+1$.  Thus $\alpha = 1-1/n$
is a $1/(1-e^{-1})$ approximation. That means, by simply choosing $\alpha =
1-1/n$, we would at most get about $1.582$ times of optimal cost. Another
obersavation of this approximation is that no matter how large $n$ is, the cost
is at most $(\rho+1) / (1-e^{-1})$. Thus the optimal cost is bounded by
constant $O(1)$ no matter how large $n$ is.

A better approximation when $n$ is large is to observe that $\lim_{n \rightarrow \infty} \alpha = 1$.
Thus we assume that $(1-a)n \approx A$ (for some constant $A$) and $\alpha^n
\approx \alpha^{n-1}$. Then we have: 
$$ n (\rho+A) \cdot \alpha^n - n(1-\alpha^n) = 0 $$ 
which gives us $\alpha = (1+\rho+A)^{-1/n}$. Put this back to
$\lim_{n \rightarrow \infty} (1-a)n = A$ we
have $\ln (1+\rho+A) = A$
which gives us
\begin{align}
A = -1-\rho-W(-1-\rho)\nonumber\\
...
$W_0(x) > -1$ when $-1 < x < 0$} branch $W_{-1}(x) < -1$.
...
\begin{figure*}
\centering
  \subfigure{\includegraphics[width=.3\linewidth]{figures/alpha_when_rho0.20.eps}}
  \subfigure{\includegraphics[width=.3\linewidth]{figures/alpha_when_rho1.00.eps}}
  \subfigure{\includegraphics[width=.3\linewidth]{figures/alpha_when_rho5.00.eps}}
  \subfigure{\includegraphics[width=.3\linewidth]{figures/C_when_rho0.20.eps}}
  \subfigure{\includegraphics[width=.3\linewidth]{figures/C_when_rho1.00.eps}}
  \subfigure{\includegraphics[width=.3\linewidth]{figures/C_when_rho5.00.eps}}
