...
C}{\partial \alpha} |_{\alpha = 0} < 0$.  When $\alpha = 1$, $\alpha$-MVA
becomes Dutch Auction thus $\rho$ must be $0$. And this is indeed a solution of
equation \ref{eq:alpha} when $\rho = 0$. Thus equation $\ref{eq:alpha}$
characterize the optimal $\alpha$ if $\alpha$-MVA is optimal.  [Increasing
...
completely determined by cost. Thus we'll only compare cost in the following.
...
\begin{itemize}
\item The optimal uniform-MVA's cost is very close optimal $\alpha$-MVA's,
especially when $n$ is large. That could be explained by 
    \begin{enumerate}
        ...
        thresholds, $\alpha^{nk}$, becomes negligible when $n$ grows large.
    \end{enumerate}
...
\item The heuristic $\alpha$-cutoff-$k$-MVA works pretty well when $\rho$ is large as
shown in figure \ref{fig:cost_comparison_.5}. But it won't be very efficient when $\rho$ is small and $n$ is
large as shown in figure \ref{fig:cost_comparison_2}.
