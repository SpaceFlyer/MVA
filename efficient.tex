...
the revenue equivalence theorem~\cite{Myerson:1981}.\footnote{Moreover, by
  ...
  in redistribution mechanisms), but this would only hurt revenue.}
...
%  \subfigure{\includegraphics[trim=0 5mm 5mm 5mm, clip, width=.3\linewidth]{figures/alpha_when_rho5.00.eps}}
  ...
  (-W(-1-\rho))^{-1/n}$. ($c = 0.1, b = \rho c$. Recall that $\rho = b/c$~for cases where $c > 0$.)}
...
\item {\em uniform $k$-MVA}:  the uniform $k$-MVA has its $k$
  ...
  corresponds to the optimal $k$ (computed by brute force search).  (Compare the fixed-step search strategy in~\cite{SarneSR2010:IncreasingSearch,DBLP:conf/iwdc/HassanJ04}.)
...
%    }
    \includegraphics[trim=0 5mm 5mm 15mm, clip, width=.65\linewidth]{figures/analyze_cost_2_5_15.eps}
    \label{fig:cost_comparison_2}
    \caption{Comparing the expected costs of different MVAs over the number of bidders $n$. ($b = 0.2$, $c=0.1$)}
