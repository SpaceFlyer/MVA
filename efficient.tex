...
it's not that negligible as our example seems to claim. In later section,
...
tiny $1$ minue cost.  But for the seller, it's a big $100$ minutes cost which
is very annoying. It's also necessary to remove bidder's bidding cost to
...
[cite] told us that the seller's gross revenue without substracting costs is
...
$\ref{eq:alpha}$ characterize the optimal $\alpha$ in all cases.  [Increasing
Threshold Search for Best-Valued Agents] has a proof for why $\alpha$-MVA is
optimal among MVAs and how to determine $\alpha$ as well. Thus if you find them
not intuitive you may reference more details there.  Our simplified cost model
...
It's difficult to get an exact closed formula for optimal $\alpha$ by equation
\ref{eq:alpha}. Thus we are going to use some simpler formulas to approximate
$\alpha$. We'll conduct experiments to compare our approximation with the
optimal $\alpha$ that's computed numerically. 
...
Now let's compare optimal $\alpha$-MVA with other kinds of MVA. In all
...
adapted to uniform distribution so this won't be a problem.  Also recall that
under this simpler model, the revenue is fixed thus the profit is
completely determined by cost. Thus we'll only compare cost.

$\alpha$-MVA can potentially have infinite many rounds. But in reality, it's
more naturaly to come up with an MVA that has finite many, say $k$ rounds at
most. Let's call them $k$-MVA where $k$ is some positive integers. One
particualr $k$- MVA is uniform $k$-MVA where the $k$ thresholds is uniformly
distributed over $[0, 1]$.  It's also known as fixed-step search strategy in
[cite:increasing search and its reference]. An optimal uniform MVA is the uniform
...
$3$ since it's too annoying to send out two broadcast messages per day (e.g. it
...
\centering
    \subfigure[$\rho = 0.5$]{
        \includegraphics[trim=0 5mm 5mm 15mm, clip, width=.4\linewidth]{figures/analyze_cost_.5_5_15.eps}
        \label{fig:cost_comparison_.5}
    }
    \subfigure[$\rho = 2$]{
        \includegraphics[trim=0 5mm 5mm 15mm, clip, width=.4\linewidth]{figures/analyze_cost_2_5_15.eps}
...
especially when $n$ is large. That's probably because
    \begin{enumerate}
...
$\rho$ is large as shown in figure \ref{fig:cost_comparison_.5}. But it's not
