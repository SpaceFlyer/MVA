...
& C(\vec a, k, \rho, n) = k \rho a_k^n / a_0^n + \nonumber\\ 
    ...
    &~= \sum_{i=0}^{k-1} \frac{a_i^n}{a_0^n} \left( \rho + \frac{a_i-a_{i+1}}{a_{i-1}} n \right)
...
It's suggested by experiments showed in figure \ref{fig:x-l} where $x$ seems to
be $1-\alpha$ for a lot of $l$.  A similar result is also occurs when we
compare the cost of different MVA: uniform $a_i$ MVA performs much worse than
$\alpha$-MVA with cutoff. In experiments, the actual optimal $k$ is very close
to our upper bound $k_\alpha$: it's mostly either $k_\alpha$ or $k_\alpha-1$.
This might be a direct consequence of $a_i = \alpha^i$'s good performance.
