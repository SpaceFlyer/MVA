...
& C(\vec a, k, \rho, n) = k \rho a_k^n / a_0^n + \nonumber\\ 
    ...
    &~= \sum_{i=1}^{k-1} \frac{a_i^n}{a_0^n} \left( \rho + \frac{a_i-a_{i+1}}{a_{i-1}} n \right)
    \label{eq:C_general}\\
...
%Introduce $a_{i, t}$ to be the $a_i$ in $t$-th iteration ($a_{i,0}$ is the
%initial guess). We modify $\frac{\partial C}{\partial a_i} = 0$ with equation
%\ref{eq:diff_C_general} to
%\begin{align}
%  & n(n+\rho)a_{i,t}^{n-2} a_{i,t+1} - n(n-1)a_{i+1,t}a_{i,t}^{n-2}-n a_{i-1,t}^{n-1} = 0 \nonumber\\
%  & \Updownarrow \nonumber \\
%  & a_{i,t+1} = \frac{(n-1)a_{i+1,t}a_{i,t}^{n-2} + a_{i-1,t}^{n-1}}{(n+\rho)a_{i,t}^{n-1}}
%\end{align}
