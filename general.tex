...
One might think that the minimum cost for this is $0$ since we don't have to know
anyone's valuation. But that cost isn't realistic: how can the mechanism ever allocate the
...
the fact that there's a maximal number of rounds to exploit the whole
reportable valuation range $[l, 1)$. Let's call that
...
\ref{eq:diff_C_general} isn't neat enough to get a piecewise linear query length.
...
of any round except the last round couldn't decrease if we increase $l$ from $0$ to
some positive value. 
...
$k$ in $O(\log k_\alpha)$ time if we can prove it's unimodal.\footnote{We
didn't prove that it's unimodal but experiments seems to support this
property.}
...
From figure \ref{fig:suboptimality}, the cost isn't unimodal over $l$ and it's quite wierd.
